...
def build_dst_dirpath(dst_dir: str, imginf: ImageInfo):
    dst_date = imginf.get_datetime().strftime("%Y%m%d")
...
def move_file(imginf: ImageInfo, dst_dir: str):
    """Move a file to a destination directory.

        The function will assure that directories missing will be created.
        The image described by imginf will be moved to the """
    src_file = imginf.get_filename()
    dst_dir_path = build_dst_dirpath(dst_dir, imginf)
...
#move_files_into_directories(files_and_dates)
    ...
    assure_dst_dir_exists(args.dst[0])
    for file in files:
        try:
            imginf = ImageInfo()
            imginf.read(file)
            move_file(imginf, args.dst[0])
